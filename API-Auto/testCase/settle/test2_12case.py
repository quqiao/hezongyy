...
class testSettlePayMent(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'url为空':  # 同上
            self.assertEqual(ss['code'], "900004")
        if self.case_name == 'orderId填写错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'orderId为空':  # 同上
            self.assertEqual(ss['code'], "900007")
        if self.case_name == 'payableAmount填写错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'payableAmount为空':  # 同上
            self.assertEqual(ss['code'], "200814")
        if self.case_name == 'orderNumber填写错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'orderNumber为空':  # 同上
            self.assertEqual(ss['code'], "200814")
