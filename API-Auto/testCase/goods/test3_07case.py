...
login_xls = readExcel.readExcel().get_xls('商品.xlsx', '7正常商品扣减库存')

@paramunittest.parametrized(*login_xls)
class testGoodsQuantity(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'url为空':  # 同上
            self.assertEqual(ss['code'], "900004")
        if self.case_name == 'id为错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'id为空':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'quantity为错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'quantity为空':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'source为正常商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'source为特卖商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'source为礼品商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'source为空':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'source为错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'type为普通商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'type为买赠换购商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'type为秒杀商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'type为特卖商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'type为阶梯价商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'type为随机特价商品':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'type为错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'type为空':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'uniqueId为错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'uniqueId为空':  # 同上
            self.assertEqual(ss['code'], "900006")
