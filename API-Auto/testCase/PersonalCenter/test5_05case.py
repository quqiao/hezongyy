...
login_xls = readExcel.readExcel().get_xls('个人中心.xlsx', '5商品列表收藏状态')
...
class testSettleAddGoodsCart(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'userId错误':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '960000')
        if self.case_name == 'userId为空':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'id列表为1个':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'id列表为2个':  # 同上
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'id列表为多个':  # 同上
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'id列表为空':  # 同上
            self.assertEqual(ss['code'], '000000')
