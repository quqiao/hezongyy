...
login_xls = readExcel.readExcel().get_xls('个人中心.xlsx', '38绑定手机号')
...
class testSettleAddGoodsCart(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'mobile正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'mobile错误':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], 'x')
        if self.case_name == 'mobile为空':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], 'x')
        if self.case_name == 'password正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'password错误':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], 'x')
