...
login_xls = readExcel.readExcel().get_xls('业务员APP.xlsx', '10添加购物车')
...
class testSettleAddGoodsCart(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'client_id正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'client_id错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'client_id为空':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'goodsId正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'goodsId错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'goodsId为空':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'quantity正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'quantity错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'quantity为空':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'addPrice正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'addPrice错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'addPrice为空':  # 同上
            self.assertEqual(ss['code'], "200915")
