...
class testSettleAddGoodsCart(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'oldPassword正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'oldPassword错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'oldPassword为空':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'newPassword正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'newPassword错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'newPassword为空':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'repNewPassword正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'repNewPassword错误':  # 同上
            self.assertEqual(ss['code'], "200915")
        if self.case_name == 'repNewPassword为空':  # 同上
            self.assertEqual(ss['code'], "200915")
