...
login_xls = readExcel.readExcel().get_xls('业务员APP.xlsx', '12删除购物车')
...
class testUserLogin(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'ids为1个':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'ids为2个':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'ids为多个':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'ids错误':  # 同上
            self.assertEqual(ss['code'], "900000")
        if self.case_name == 'ids为空':  # 同上
            self.assertEqual(ss['code'], "900000")
