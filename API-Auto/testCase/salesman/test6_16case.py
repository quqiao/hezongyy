...
login_xls = readExcel.readExcel().get_xls('业务员APP.xlsx', '16结算')
...
class testSettleAddGoodsCart(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'root为1个':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'root为2个':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'root为多个':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'root错误':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'root为空':  # 同上
            self.assertEqual(ss['code'], "000000")
