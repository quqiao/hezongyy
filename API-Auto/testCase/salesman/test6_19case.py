...
class testSettleAddGoodsCart(unittest.TestCase):
    ...
    def checkResult(self):  # 断言
        ...
        if self.case_name == 'userId为空':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'goodsIds为1个':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'goodsIds为2个':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'goodsIds为多个':  # 同上
            self.assertEqual(ss['code'], "000000")
        if self.case_name == 'remarks正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'remarks错误':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'remarks为空':  # 同上
            self.assertEqual(ss['code'], "900006")
        if self.case_name == 'settleType正确':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(ss['code'], '000000')
        if self.case_name == 'settleType错误':  # 同上
            self.assertEqual(ss['code'], "900004")
        if self.case_name == 'settleType为空':  # 同上
            self.assertEqual(ss['code'], "900004")
