...
class BasePage(object):
    ...
    def _open(self, url, pagetitle):
        # 使用get打开访问链接地址
        if (url == "http://192.168.20.80:9000/#/login"):
            self.driver.get(url)
            self.driver.maximize_window()
            sleep(2)
        else:
            self.login("liuquan","123456")
            self.driver.get(url)
    ...
    def login(self, uname, pwd):
        self.driver.get("http://192.168.20.80:9000/#/login")
        self.driver.maximize_window()
        self.find_element(By.XPATH, "//input[@placeholder='请输入用户名']").send_keys(uname)
        self.find_element(By.XPATH, "//input[@placeholder='请输入密码']").send_keys(pwd)
        self.find_element(By.XPATH, "//button/span").click()
        sleep(2)
